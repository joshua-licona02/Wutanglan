 ...
 if($result->num_rows > 0){
	...
	if($result->num_rows > 0){
 	...
 	for($i = 0; $i<count($accountability_ids); $i++){
 		...
 		$sql = "UPDATE accountability SET date = '$date', time = '$time', course_id = '$course_id', cadet_id = '$current_cadet', status = '$current_status', comments = '$current_comment', submitted_by = '$id_number', submitted_by_role = 'Cadet' WHERE accountability.accountability_id='$current_id'";
 		if($conn->query($sql)){
 			continue;
		}
		else{
