...
$sql = "SELECT * FROM cadets JOIN rank on cadets.rank=rank.rank join course_enrollment on course_enrollment.cadet_id = cadets.id_number where course_id = '$course_id' order by rank_id, class, last_name";
 ...
 if($result->num_rows > 0){
	...
	$sql = "INSERT INTO `accountability` (`accountability_id`, `date`, `time`, `course_id`, `cadet_id`, `status`, `comments`, `submitted_by`, `submitted_by_role`) VALUES (NULL, '$date', '$time', '$course_id', '$cadet_id', '$cadet_status', '$comment', '$id_number', 'Professor')";
...
echo "<script> alert('Accountability Submitted!'); window.location = 'profHome.php';</script>";
